...
def move_chip(matrix, command, old_coordinates):
        ...
        if command == "Up":
            ...
            if new_coordinates[0] >= 0:
                move_element_numpy(matrix, old_coordinates, new_coordinates)
            else:
...
def main(matrix, chip_uid, chip_move):
    ...
    """
    global new_coordinates
    g = 0 #кол-во прохожденных клеток с 2
    ...
    if g > 0:
        is_continue_move = input(f"Фишка прошла по пропости {g} раз. Продолжить? (Введите Да)")
        if is_continue_move == "Да":
            return matrix
        else:
            return old_matrix
    else:
